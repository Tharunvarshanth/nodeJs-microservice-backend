...
K8S_VERSION=1.22
REGION=ap-south-1
BASE_REPO_NAME=$(PROJECT_NAME)-$(ENVIRONMENT)

AWS_ACCOUNT_ID?=$(shell aws sts get-caller-identity --query "Account" --output text)
IMAGE_REGISTRY?=$(AWS_ACCOUNT_ID).dkr.ecr.$(REGION).amazonaws.com

ORDERS_SERVICE_IMAGE_REPO_NAME=$(BASE_REPO_NAME)/$(ORDERS_SERVICE_NAME)
ORDERS_SERVICE_IMAGE_REPO_URL=$(IMAGE_REGISTRY)/$(ORDERS_SERVICE_IMAGE_REPO_NAME):$(ORDERS_SERVICE_VERSION)

CUSTOMERS_SERVICE_IMAGE_REPO_NAME=$(BASE_REPO_NAME)/$(CUSTOMERS_SERVICE_NAME)
CUSTOMERS_SERVICE_IMAGE_REPO_URL=$(IMAGE_REGISTRY)/$(CUSTOMERS_SERVICE_IMAGE_REPO_NAME):$(CUSTOMERS_SERVICE_VERSION)
...
create-registry-repositories:
	...
	- aws ecr create-repository --repository-name $(ORDERS_SERVICE_IMAGE_REPO_NAME) --region $(REGION)

delete-registryrepositories:
	- aws ecr delete-repository --force --repository-name $(CUSTOMERS_SERVICE_IMAGE_REPO_NAME) --region $(REGION)
	- aws ecr delete-repository --force --repository-name $(ORDERS_SERVICE_IMAGE_REPO_NAME) --region $(REGION)
